...
contract LibPolicyContractInteractionTest is PolicyLibrariesSuiteBase {
    /// @dev [LPCI-1] Valid destination + valid function proof + valid params returns true.
    ...
    /// @dev [LPCI-2] Destination disallowed returns false.
    ...
    /// @dev [LPCI-3] Function disallowed returns false.
    ...
    /// @dev [LPCI-4] Parameter constraints mismatch returns false.
    ...
    /// @dev [LPCI-5] `anyFunction == true` bypasses function-proof check but still enforces destination + params.
    ...
    /// @dev [LPCI-6] Empty constraints can be valid when function leaf uses empty constraints hash.
    ...
    /// @dev [LPCI-7] Same selector with different constraints hash must not validate with old proof.
    ...
    /// @dev [LPCI-8] Data shorter than selector is rejected when function filtering is required.
    ...
    /// @dev [LPCI-9] Desired behavior: malformed constraints payload fails closed instead of reverting.
    ...
    /// @dev [LPCI-FN-1] `anyFunction == true` returns true regardless of data/proof.
    ...
    /// @dev [LPCI-FN-2] `anyFunction == false` and `data.length < 4` returns false.
    ...
    /// @dev [LPCI-FN-3] Valid (selector, constraintsHash) proof returns true.
    ...
    /// @dev [LPCI-FN-4] Invalid function proof returns false.
    ...
    /// @dev [LPCI-FN-5] Selector tampering invalidates previously valid proof.
    ...
    /// @dev [LPCI-FN-6] Constraints tampering invalidates proof via changed constraints hash.
    ...
    /// @dev [LPCI-LEAF-1] Golden vector for known selector/hash produces expected leaf.
    ...
    /// @dev [LPCI-LEAF-2] Deterministic for identical inputs.
    ...
    /// @dev [LPCI-LEAF-3] Changing selector or constraintsHash changes leaf.
    ...
    /// @dev [LPCI-LEAF-4] Leaf uses double-hash construction, not single-hash variant.
