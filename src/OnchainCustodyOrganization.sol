...
contract OnchainCustodyOrganization {
        ...
        returns (bool)
    {
        ...
        if (
            policy.initiatorType == Policies.ApproverType.Member
                && addressToMemberId[initiatorAddress] == policy.initiatorId
        ) return true;

        // Case: The policy matches transactions made by any individual from a specific group, and the initiator
        //       is in that group
        if (
            policy.initiatorType == Policies.ApproverType.Group && isMemberInGroup(initiatorAddress, policy.initiatorId)
        ) return true;
        ...
        returns (bool)
    {
        ...
        if (policy.destinationType == Policies.DestinationType.WhitelistedOnly) {
            return isAddressWhitelisted(actualDestination);
        }
    ...
    function isSignerAuthorizedForPolicy(Policies.Policy memory policy, address signer) external view returns (bool) {
        ...
        uint8 memberId = addressToMemberId[signer];
        ...
        if (policy.approverType == Policies.ApproverType.Group) {
            return isMemberInGroup(memberId, policy.approverId);
        }
        ...
        returns (bool)
    {
        ...
        if (policy.initiatorType == Policies.ApproverType.Group) {
            return isMemberInGroup(memberId, policy.initiatorId);
        }
