...
> - `SafeExecutorModule` is the only module designed and audited for this role. Its restrictions (see [The SafeExecutorModule](#the-safeexecutormodule) above) strictly limit the capabilities of the Authorized Executor's signing key. A general-purpose Safe module does not provide those guarantees.
