...
| FMU-LEAF-9 | `src/libraries/MerkleUtils.sol` | `computeAddressLeaf` | Deterministic and double-hashed leaf format for random addresses | 2000 | P1 |
| FMU-LEAF-10 | `src/libraries/MerkleUtils.sol` | `computeAddressLeaf` | Distinct random addresses do not collide in practical fuzz space | 2000 | P1 |
...
| FTTU-AMOUNT-15 | `src/libraries/TokenTransferUtils.sol` | `extractTransferAmount` | Native path returns `value`; ERC-20 path returns encoded amount | 10000 | P0 |
...
| FTTU-TOKEN-17 | `src/libraries/TokenTransferUtils.sol` | `extractTokenAddress` | Non-empty calldata always returns `to`; empty calldata always returns `address(0)` | 10000 | P0 |
| FCIU-SELECT-18 | `src/libraries/ContractInteractionUtils.sol` | `extractFunctionSelector` | For random calldata length `>= 4`, selector always equals first 4 bytes | 1000 | P2 |
...
| FTU-DUR-20 | `src/libraries/TimelockUtils.sol` | `validateTimelockDurationOrRevert` | Any duration in `[2 days, 30 days]` always succeeds | 2000 | P1 |
| FTU-DUR-21 | `src/libraries/TimelockUtils.sol` | `validateTimelockDurationOrRevert` | Any duration outside `[2 days, 30 days]` always reverts | 2000 | P1 |
...
| FLOAOT-INIT-23 | `src/organization/libraries/LibOrganizationAdminOperationTimelock.sol` | `initializeAdminOperationTimelock`, `getAdminOperationTimelockDurationSeconds` | Valid boundaries (`2 days`, `30 days`) persist exactly; out-of-range values always revert via timelock validation | 2000 | P1 |
| FLOAOT-EXPIRY-24 | `src/organization/libraries/LibOrganizationAdminOperationTimelock.sol` | `validateTimelockExpiredOrRevert`, `computeCanFinalizeAtTimestamp` | Boundary semantics hold: `< canFinalizeAt` reverts, `>= canFinalizeAt` succeeds, and computed timestamp equals `block.timestamp + configuredDuration` | 10000 | P0 |
| FOAOTB-DUR-25 | `src/organization/base/OrganizationAdminOperationTimelockBase.sol` | `adminOperationTimelockDurationSeconds` | Base getter always reflects initialized value and remains unchanged across guardian/recovery operation flows | 2000 | P1 |
...
| FOPX-CTOR-29 | `src/organization/OrganizationProxy.sol` | constructor | Constructor always seeds deployer and whitelist storage slots exactly to constructor inputs for later initialization/upgrade auth checks | 2000 | P1 |
| FLOS-NONCE-30 | `src/organization/libraries/LibOrganizationSignatures.sol` | `computeNonce` | `(operationType, operationData, salt)` sensitivity: changing any field changes nonce | 10000 | P0 |
| FLOS-NONCE-31 | `src/organization/libraries/LibOrganizationSignatures.sol` | `computeNonce` | Cross-organization isolation: same inputs on different org addresses produce different nonces | 10000 | P0 |
...
| FOSB-NONCE-34 | `src/organization/base/OrganizationSignaturesBase.sol` | `computeNonce`, `isNonceUsed` | Base view functions stay library-consistent: nonce derivation caller-independent and nonce visibility reflects actual consume/revert outcomes | 2000 | P1 |
| FLOEIP-DOMAIN-35 | `src/organization/libraries/LibOrganizationEIP712.sol` | `getDomainSeparator` | Domain separator deterministic for same org+chain, different for other org/chain | 10000 | P0 |
...
| FLOEIP-TYPE-37 | `src/organization/libraries/LibOrganizationEIP712.sol` | typehash constants | All message type hashes are unique and match documented type strings | 2000 | P1 |
...
| FLOA-HAR-44 | `src/organization/libraries/LibOrganizationAdmin.sol` | private helpers via harness | `_getAdminOperationHash` deterministic + field-sensitive; `_areAdminSignaturesValid` robust under mixed signature streams | 2000 | P1 |
...
| FLOP-PLEAF-62 | `src/organization/libraries/LibOrganizationPolicy.sol` | private `_computePolicyLeaf` via harness | Policy leaf is deterministic, double-hashed, and sensitive to every field | 2000 | P1 |
...
| FLPD-DEST-64 | `src/organization/libraries/policy/LibPolicyDestination.sol` | `getActualDestination` | ERC-20 transfer uses token recipient as destination; non-transfer uses `to` | 10000 | P0 |
...
| FLPT-TOKEN-66 | `src/organization/libraries/policy/LibPolicyTokenTransfer.sol` | `_isTokenAllowedByPolicy` | `anyToken` and specific-token behavior are enforced exactly | 10000 | P0 |
...
| FLPCI-FLEAF-71 | `src/organization/libraries/policy/LibPolicyContractInteraction.sol` | private `_computeFunctionLeaf` via harness | Deterministic, double-hashed function leaf generation | 2000 | P1 |
...
| FLPA-REQ-79 | `src/organization/libraries/policy/LibPolicyApproval.sol` | `getRequiredApprovals` | Member approver requires exactly 1; group approver requires threshold | 10000 | P0 |
...
| FLPRL-USAGE-83 | `src/organization/libraries/policy/LibPolicyRateLimits.sol` | `computeUsageKey` | Scope composition (`AcrossAll` vs `PerEntity`) produces expected key isolation/merging | 10000 | P0 |
...
| FLOAT-HASH-90 | `src/organization/libraries/LibOrganizationAccountTransaction.sol` | private hash helpers via harness | `_computeInitiatorHashFromParams` and `_computeReviewHashFromParams` are field-sensitive and deterministic | 10000 | P0 |
| FLOAT-HASH-91 | `src/organization/libraries/LibOrganizationAccountTransaction.sol` | private hash helpers via harness | Review hash always changes when `initiatorSignature` changes | 10000 | P0 |
...
| FLOAS-RHASH-101 | `src/organization/libraries/LibOrganizationAccountSignature.sol` | `_getReviewSignatureHash` | Review hash binds to initiator signature (`keccak256(initiatorSignature)`) | 10000 | P0 |
...
| FLOGU-INIT-105 | `src/organization/libraries/LibOrganizationGuardian.sol` | `initiateGuardianUpdate` | Pending guardian and finalize timestamp set correctly | 2000 | P1 |
...
| FOAFB-IMPL-123 | `src/organization/base/OrganizationAccountFactoryBase.sol` | `implementation` | Beacon implementation getter always reverts when unset and returns exactly the configured whitelisted account implementation once set | 10000 | P0 |
...
| FLOAF-ADDRESS-126 | `src/organization/libraries/LibOrganizationAccountFactory.sol` | `computeAccountAddress` | Same salt across different organizations yields different account addresses | 2000 | P1 |
...
| FLOAF-BYTECODE-128 | `src/organization/libraries/LibOrganizationAccountFactory.sol` | `_getAccountProxyBytecode` via harness | Bytecode generation is deterministic and organization-address-sensitive (changes across org addresses, stable within one org) | 2000 | P1 |
...
| FOF-CTOR-132 | `src/organization/OrganizationFactory.sol` | constructor | Constructor always rejects `address(0)` deployer and accepts non-zero deployer as immutable deployment authority | 2000 | P1 |
| FOF-BYTECODE-133 | `src/organization/OrganizationFactory.sol` | private `_getOrganizationProxyBytecode` via harness | Bytecode is deterministic and field-sensitive to `(implementationAddress, whitelistAddress)` and matches CREATE2 precompute inputs | 2000 | P1 |
...
| FIWI-VALIDATE-139 | `src/implementation-whitelist/ImplementationWhitelistImplementation.sol` | `isImplementationWhitelisted`, `validateIsImplementationWhitelistedOrRevert` | Validation helper is mapping-consistent for random `(contractType, implementation)` tuples (true passes, false always reverts) | 10000 | P0 |
...
| FAI-SIG-147 | `src/account/AccountImplementation.sol` | `isValidSignature` | Account always delegates signature validation to organization result | 2000 | P1 |
| FAI-RECEIVE-148 | `src/account/AccountImplementation.sol` | `receive`, `getOrganizationAddress` | Receiving native token always succeeds/emits and never mutates stored organization address returned by getter | 2000 | P1 |
| FSEM-CTOR-149 | `src/safe-module/SafeExecutorModule.sol` | constructor | Any zero-address constructor arg (`safe`, `authorizedExecutor`, `batchedTransaction`) always reverts; non-zero tuple initializes immutables exactly | 2000 | P1 |
...
| FSEM-EXEC-152 | `src/safe-module/SafeExecutorModule.sol` | `executeOnBehalf` | Delegatecall used iff target is `BATCHED_TRANSACTION`; otherwise regular call | 2000 | P1 |
...
| FSEM-SIG-155 | `src/safe-module/SafeExecutorModule.sol` | `isValidSignature` | Returns magic only when recovered signer equals `AUTHORIZED_EXECUTOR` | 10000 | P0 |
...
| FCF-FDINIT-168 | `cross-file` | factory deploy + initialization | Successful factory deployment always yields initialized org state in the same tx with `getDeployerAddress() == OrganizationFactory` | 10000 | P0 |
