...
flowchart TB
    ...
    subgraph Step4["Step 4: Rate Limits"]
        rateLimitCheck["_validateAndUpdateRateLimitOrRevert()<br/>(LibOrgAcctTx:241-276)"]
        checkLimitType{"policy.config.rateLimit<br/>.limitType == TimeInterval?"}
        ...
        updateLimit["LibOrganizationPolicy.checkAndUpdateRateLimit()<br/>→ LibPolicyRateLimits"]
    end
    ...
    checkPolicyType -->|AutoApprove| rateLimitCheck
    checkApprovals --> rateLimitCheck

    rateLimitCheck --> checkLimitType
    checkLimitType -->|Yes| getDestination
...
flowchart TB
    ...
    subgraph UniqueExecute["Unique to executeAccountTransaction"]
        rateLimits["_validateAndUpdateRateLimitOrRevert()"]
        updateUsage["LibOrganizationPolicy<br/>.checkAndUpdateRateLimit()"]
...
| `LibOrganizationPolicy.checkAndUpdateRateLimit()` | ✓ | - | - |
...
| **Modifies state** | Yes (nonce consumed, rate limits updated) | Yes (nonce consumed only) |
| **Rate limits** | Validates and updates usage tracking | No rate limit processing |
...
| **Rate limits** | Not supported (view function) | Supported |
...
| `LibOrganizationAccountTransaction.sol` | 241-276 | `_validateAndUpdateRateLimitOrRevert()` |
